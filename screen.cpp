...
void Screen::draw_board() {
    ...
    double *board_float = board_obj->get_board_float();
    if(draw_smooth) {
        int color;
        if(draw_colors) {
            for(int j = 0; j < CELL_HEIGHT; j++) {
                for(int i = 0; i < CELL_WIDTH; i++) {
                    color = colors[((board[j*CELL_WIDTH+i]+alive_offset)/color_speed_divisor) % 256];
                    set_pixel(i, j, color);
                }
            }
        }
            ...
            for(int j = 0; j < CELL_HEIGHT; j++) {
                for(int i = 0; i < CELL_WIDTH; i++) {
                    color = 256 * board_float[j*CELL_WIDTH+i];
